...
type ForecastDistribution={method:string;residualCount:number;residualMean:number;residualSd:number;residualSkew:number;tailAsymmetry:number|null;empiricalBlend:number;residualQuantiles?:Record<string,number|null>};
type Forecast={seriesId:string;title:string;economy:string;family:string;latest:number;forecast:number;delta:number;modelWeights?:Record<string,number>;walkForwardRmse?:Record<string,number|null>;validationPoints?:number;modelAgreement?:number;calibrationConfidence?:number;modelDispersion?:number;uncertainty?:number;driftScore?:number;driftStatus?:string;forecastBias?:number;distribution?:ForecastDistribution};
...
const distributionLabel=(method?:string)=>method==='walk-forward-empirical-residuals'?'Empirical residuals':method==='hybrid-parametric'?'Hybrid fallback':'Not calibrated';
...
export function AdaptiveResearchPanel({sources=[],forecasts=[]}:{sources?:SourceReliability[];forecasts?:Forecast[]}){
  ...
  return <>
    <section className="section-head adaptive-research-head"><div><span className="eyebrow">Adaptive Research Quality</span><h2>Models and sources must earn influence</h2><p>Forecast weights are learned from walk-forward errors. Recent deterioration reduces confidence automatically. When residual history is deep enough, probability and uncertainty bands use the observed forecast-error distribution rather than assuming symmetric normal errors.</p></div></section>
    <section className="adaptive-research-grid">
      ...
      <article className="panel adaptive-forecast-panel">
        ...
        <div className="adaptive-forecast-list">{calibrated.map(row=>{
          const weights=Object.entries(row.modelWeights||{}).sort((a,b)=>b[1]-a[1]),dist=row.distribution;
          return <div className="adaptive-forecast-row" key={row.seriesId}>
            ...
            {dist&&<div className="adaptive-validation"><span>{distributionLabel(dist.method)}</span><span>{dist.residualCount} residuals</span><span>Skew {format(dist.residualSkew)}</span><span>Tail ratio {format(dist.tailAsymmetry)}</span><span>Empirical weight {pct(Number(dist.empiricalBlend||0))}</span></div>}
          </div>;
        })}</div>
      </article>
    </section>
