    ...
    <section className="adaptive-research-grid">
      ...
      <article className="panel adaptive-forecast-panel">
        ...
        <div className="adaptive-forecast-list">{calibrated.map(row=>{
          ...
          return <div className="adaptive-forecast-row" key={row.seriesId}><div className="adaptive-forecast-main"><div><strong>{row.seriesId}</strong><small>{row.title} · {row.economy}</small></div><b className={Number(row.delta)>=0?'positive':'negative'}>{Number(row.delta)>0?'+':''}{format(row.delta)}</b></div><div className="adaptive-weights">{weights.map(([model,weight])=><span key={model}>{model} <b>{pct(weight)}</b></span>)}</div><div className="adaptive-validation"><span>{row.validationPoints} walk-forward tests</span><span>Agreement {pct(Number(row.modelAgreement||0))}</span><span>Calibration {pct(Number(row.calibrationConfidence||0))}</span><span>Uncertainty {format(row.uncertainty)}</span></div></div>;
        })}</div>
      </article>
    </section>
  </>;
