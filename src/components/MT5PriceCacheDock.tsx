...
import { fetchMT5CacheStatus,MT5_FX_PAIRS,type MT5CacheStatus,type MT5SeriesStatus } from '../lib/mt5-price-cache';
...
function PairCard({row,pair}:{row:MT5SeriesStatus|undefined;pair:string}){
  ...
  const batch=row.lastBatch;
  const storedRatio=row.rawBytes>0?row.compressedBytes/row.rawBytes:0;
  return <article className={`mt5-db-pair ${healthTone(row.health)}`}>
    <div className="mt5-db-pair-head"><div><strong>{pair}</strong><span>{row.brokerSymbol||pair} · M1 canonical</span></div><b>{row.health.replaceAll('_',' ')}</b></div>
    <div className="mt5-db-price"><div><span>Latest MT5 close</span><strong>{price(row)}</strong><small>{row.marketOpen?'MARKET OPEN':'MARKET CLOSED'} · latency {ageMs(row.freshnessMs)}</small></div><div className="mt5-db-integrity-orb" style={{'--integrity':`${Math.min(100,row.integrityScore)}%`} as CSSProperties}><strong>{row.integrityScore}</strong><span>INTEGRITY</span></div></div>
    ...
    <div className="mt5-db-cumulative"><span>Lifetime dedupe <b>{fmt(row.deduplicatedBars,0)}</b></span><span>Retired bars <b>{fmt(row.evictedBars,0)}</b></span><span>Retired chunks <b>{fmt(row.evictedChunks,0)}</b></span><span>Broker changes <b>{fmt(row.brokerSymbolChanges,0)}</b></span></div>
    {row.alerts.length>0?<div className="mt5-db-alerts">{row.alerts.map((alert,i)=><span key={`${pair}-${i}`}>{alert}</span>)}</div>:<div className="mt5-db-clean">✓ No current integrity alarms</div>}
  </article>;
}
...
export function MT5PriceCacheDock(){
  ...
  const state=tone(utilization),health=status?.databaseHealth?.state||'WAITING';
  ...
  return <div className={`mt5-cache-dock ${open?'open':''} ${state}`}>
    <button className="mt5-cache-toggle" onClick={()=>setOpen(v=>!v)} title="MT5 rolling FX database operations"><span className={`mt5-cache-pulse ${healthTone(health)}`}></span><div><strong>MT5 PRICE DATABASE</strong><small>{loading?'SYNCING':status?`${status.databaseHealth.pairsOnline}/4 LIVE · ${pct(utilization)}`:'OFFLINE'}</small></div><b>{open?'×':'DB'}</b></button>
    {open&&<div className="mt5-cache-panel"><div className="mt5-cache-head"><div><span>FXGA · METATRADER 5 · DATABASE OPERATIONS</span><h3>Rolling Price Data Management Console</h3><p>Four-pair M1 canonical store · HTF derived on demand · bounded FIFO retention</p></div><div className="mt5-cache-head-actions"><span className={`mt5-db-health ${healthTone(health)}`}>{health.replaceAll('_',' ')}</span><button onClick={()=>void load()}>REFRESH</button></div></div>
      ...
      {!status?<div className="mt5-cache-loading">Waiting for MT5 database state…</div>:<>
        <section className="mt5-cache-hero"><div><span>Compressed database payload</span><strong>{mb(status.totalCompressedBytes)} <small>/ 200.00 MB envelope</small></strong><p>{mb(status.freeEnvelopeBytes)} free in application envelope · {mb(status.freeToHardBytes)} before protected payload ceiling.</p><div className="mt5-db-governor-label"><b>{status.management.governorState}</b><span>{status.management.nextAction}</span></div></div><div className={`mt5-cache-ring ${state}`} style={{'--p':`${Math.min(100,utilization)}%`} as CSSProperties}><b>{pct(utilization)}</b><small>USED</small></div></section>
        ...
        <div className="mt5-cache-kpis upgraded"><div><span>Bars retained</span><strong>{fmt(status.totalBars,0)}</strong><small>{fmt(status.totalChunks,0)} UTC-day chunks</small></div><div><span>Raw OHLCV equivalent</span><strong>{mb(status.totalRawBytes)}</strong><small>{savings.toFixed(1)}% compression saving</small></div><div><span>FIFO retired</span><strong>{fmt(status.evictedBars||0,0)} bars</strong><small>{fmt(status.evictedChunks||0,0)} chunks · {mb(status.evictedBytes||0)}</small></div><div><span>Pair health</span><strong>{status.databaseHealth.pairsHealthy}/{status.databaseHealth.pairsExpected}</strong><small>{status.databaseHealth.integrityIssues} active notices</small></div><div><span>Last ingest</span><strong>{age(status.lastIngestAt)}</strong><small>{status.baseTimeframe} source</small></div><div><span>Derived frames</span><strong>{status.derivedTimeframes?.length||0}</strong><small>{status.derivedTimeframes?.filter(x=>x!=='M1').join(' · ')}</small></div></div>
        <div className="mt5-cache-policy upgraded"><span className={status.management.compressionActive?'on':''}><b>COMPRESS</b> gzip level 9</span><span className={status.management.deduplicationActive?'on':''}><b>DEDUPE</b> timestamp-safe</span><span className={status.management.evictionArmed?'on':''}><b>FIFO</b> oldest-first armed</span><span className={status.management.integrityMonitoring?'on':''}><b>INTEGRITY</b> automatic audit</span><span className="on"><b>HTF</b> derived, not duplicated</span></div>
