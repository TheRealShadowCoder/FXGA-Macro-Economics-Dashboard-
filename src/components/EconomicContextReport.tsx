...
const evidenceScore=(state:EconomyMacroState)=>Math.max(0,Math.min(100,Number(state.confidence||0)));
const coverageScore=(state:EconomyMacroState)=>state.coverageRatio!=null?Math.max(0,Math.min(1,state.coverageRatio)):Math.max(.2,Math.min(1,state.observationCount/12));
const qualityAdjustedCurrencyScore=(state:EconomyMacroState)=>state.currencyScore*coverageScore(state)*(state.averageEvidenceQuality??Math.max(.25,evidenceScore(state)/100));
...
function CompareAll({economies,onSelect}:{economies:EconomyMacroState[];onSelect:(id:string)=>void}){
  const ranked=[...economies].sort((a,b)=>qualityAdjustedCurrencyScore(b)-qualityAdjustedCurrencyScore(a));
  return <>
    <section className="economy-world-summary"><div><span className="eyebrow">All Economies</span><h2>Global economic comparison</h2><p>This compares every economy currently covered by the macro engine. The ordering is <strong>quality-adjusted</strong>: a strong raw macro score with thin evidence is deliberately penalised. Click an economy to learn what is driving it.</p></div><div className="economy-world-count"><strong>{economies.length}</strong><span>economies currently analysed</span></div></section>
    <section className="economy-comparison-grid">{ranked.map((e,index)=>{const adjusted=qualityAdjustedCurrencyScore(e);return <button key={e.id} onClick={()=>onSelect(e.id)} className="economy-compare-card"><div className="economy-compare-rank">#{index+1}</div><div><small>{e.currency} · {e.centralBank}</small><h3>{e.label}</h3><p>{e.regime}</p></div><div className={`economy-compare-score ${scoreTone(adjusted)}`}><strong>{adjusted>0?'+':''}{adjusted.toFixed(0)}</strong><span>quality-adjusted</span></div><div className="economy-compare-meta"><span>Policy <b>{e.policyStance}</b></span><span>Evidence <b>{evidenceScore(e)}/100</b></span><span>Data <b>{e.observationCount}</b></span><span>Status <b>{e.reportStatus||'standard'}</b></span></div></button>;})}</section>
...
export function EconomicContextReport({dashboard,analysis,loading,error}:Props){
  ...
  return <section className="economic-context-report">
    <section className="economy-selector-panel">
      <div><span className="eyebrow">Choose an economy</span><h2>Economic Context Report</h2><p>Select <strong>All Economies</strong> for the world comparison or choose one economy for a detailed, beginner-friendly explanation. Evidence scores describe data quality and agreement; they are <strong>not probabilities that the forecast will be correct</strong>.</p></div>
    ...
    {selected==='ALL'?<CompareAll economies={economies} onSelect={id=>setSelected(id)}/>:state&&story?<>
      {state.reportStatus==='partial'?<div className="economic-context-note warn"><strong>Partial economy report.</strong> FRED evidence is currently missing {state.missingDimensions?.join(', ')||'one or more macro dimensions'}. The evidence score is capped and the missing data is not estimated.</div>:null}
      <section className="economic-context-hero"><div><span className="eyebrow">{state.label} · {state.currency} · Plain English</span><h2>{story.headline}</h2><p>{story.story}</p></div><div className="economic-context-confidence"><small>EVIDENCE CONFIDENCE</small><strong>{evidenceScore(state)}/100</strong><span>{state.observationCount} verified observations · quality/coverage score, not probability</span></div></section>
      ...
      <section className="economy-beginner-summary"><article><small>1 · ECONOMY</small><strong>{state.label}</strong><span>{state.regime}</span></article><article><small>2 · CENTRAL BANK</small><strong>{state.centralBank}</strong><span>{state.policyStance} policy stance</span></article><article><small>3 · CURRENCY</small><strong>{state.currency}</strong><span>{state.currencyBias} macro bias · raw score {state.currencyScore}</span></article><article><small>4 · EVIDENCE</small><strong>{evidenceScore(state)}/100</strong><span>{state.reportStatus||'standard'} coverage · {state.observationCount} observations</span></article></section>
      ...
      <section className="economic-context-cards educational">{dimensions.map((d:EconomyDimension)=>{const help=dimensionHelp[d.id]??{question:'What is happening?',means:'This is one part of the macro economy.',high:'A high score means supportive evidence.',low:'A low score means weak evidence.'};return <article className={`economic-context-card ${scoreTone(d.score)}`} key={d.id}><header><div><small>{d.label.toUpperCase()}</small><h3>{plainScore(d.score)}</h3></div><b>{signed(d.score)}</b></header><div className="economy-question">{help.question}</div><p>{help.means}</p><div className="economic-why"><strong>What the current score means</strong><span>{d.score>=0?help.high:help.low}</span></div><div className="economy-score-guide"><span>-100 weak</span><div><i style={{left:`${Math.max(0,Math.min(100,(d.score+100)/2))}%`}}/></div><span>+100 strong</span></div>{d.contributors.length?<div className="economic-drivers"><small>DATA DRIVING THIS</small>{d.contributors.slice(0,5).map(item=><span key={item.seriesId} title={item.seriesId}>{item.title}{item.quality!=null?` · quality ${Math.round(item.quality*100)}/100`:''}</span>)}</div>:<div className="economic-drivers"><small>DATA DRIVING THIS</small><span>No verified evidence is currently available for this dimension.</span></div>}</article>;})}</section>
