...
export function EconomicContextReport({dashboard,analysis,loading,error}:Props){
  ...
  return <section className="economic-context-report">
    ...
    <section className="economic-market-impact"><div className="economic-context-section-head"><div><span className="eyebrow">What this means for markets</span><h3>Current macro bias by asset</h3></div></div>{assetViews.length?<div className="economic-asset-grid">{assetViews.map(asset=><article key={asset.id}><span>{asset.label}</span><strong>{asset.bias}</strong><b className={asset.score>0?'up':asset.score<0?'down':'flat'}>{signed(asset.score)}</b><small>Macro confidence {analysis?.confidence??'—'}%</small></article>)}</div>:<div className="economic-context-empty">Asset-level macro implications are still loading.</div>}<p className="economic-disclaimer">These are current macro tendencies from the dashboard's evidence model, not guarantees that an asset must rise or fall today. Price confirmation and event risk still matter.</p></section>
