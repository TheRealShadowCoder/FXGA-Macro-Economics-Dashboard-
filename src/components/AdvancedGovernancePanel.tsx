...
export function AdvancedGovernancePanel({data}:{data?:Core|null}){
  const pairs=(data?.pairDecisions||[]).filter(p=>p.reactionFunctionGap||p.crossAsset||p.evidenceCompleteness||p.structuralBreak||p.horizonCalibration||p.historicalAnalogues);
  ...
  return <>
    ...
    <section className="advanced-governance-grid">{pairs.map(pair=>{
      const reaction=pair.reactionFunctionGap,cross=pair.crossAsset,complete=pair.evidenceCompleteness,structural=pair.structuralBreak,horizon=pair.horizonCalibration,change=pair.decisionChange,analogues=pair.historicalAnalogues;
