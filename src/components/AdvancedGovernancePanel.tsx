...
type Pair={
  ...
  crossAsset?:{available:boolean;score:number;alignment:string;status:string;availableFactors:number;used?:Array<{id:string;weight:number;score:number;changePercent:number}>};
...
const quality=(status?:string)=>status==='complete'||status==='aligned'||status==='usable'||status==='calibrated'||status==='stable'?'positive':status==='insufficient'||status==='adverse'||status==='fresh-flip'||status==='strong-repricing-conflict'||status==='opposed'?'negative':'neutral';
...
export function AdvancedGovernancePanel({data}:{data?:Core|null}){
  const pairs=(data?.pairDecisions||[]).filter(p=>p.reactionFunctionGap||p.crossAsset||p.evidenceCompleteness||p.horizonCalibration||p.historicalAnalogues);
  if(!pairs.length)return null;
  return <>
    <section className="section-head advanced-governance-head"><div><span className="eyebrow">Institutional Decision Audit</span><h2>Repricing, completeness, calibration and analogue evidence</h2><p>The second-pass audit separates future policy repricing, independent cross-asset transmission, missing research evidence and realized historical skill. None of these layers can manufacture confirmation when their underlying evidence is absent.</p></div></section>
    <section className="advanced-governance-grid">{pairs.map(pair=>{
      const reaction=pair.reactionFunctionGap,cross=pair.crossAsset,complete=pair.evidenceCompleteness,horizon=pair.horizonCalibration,change=pair.decisionChange,analogues=pair.historicalAnalogues;
      return <article className="panel advanced-governance-card" key={pair.symbol}>
        <div className="advanced-governance-title"><h3>{pair.symbol}</h3><div>{complete&&<span className={quality(complete.status)}>Evidence {complete.score}%</span>}{horizon&&<span className={quality(horizon.status)}>Calibration {Math.round(horizon.overallCalibratedProbability*100)}%</span>}</div></div>
        ...
        <details className="advanced-governance-details"><summary>Audit evidence</summary><div>
          ...
          {cross?.used?.length?<><strong>Cross asset factors</strong>{cross.used.map(x=><p key={x.id}>{x.id}: change {n(x.changePercent,2)}%, transformed score {sign(x.score)}, model weight {n(x.weight,2)}.</p>)}</>:null}
