...
function FailureRow({ item }: { item: MacroFailureSeries }) {
  return <div className="dq-failure-row" data-explain-key="missing-data-coverage">
...
export function DataQualityPanel({ refreshKey = '' }: { refreshKey?: string }) {
  ...
  return <section className="dq-workspace">
    ...
    <div className="dq-kpi-grid">
      <article data-explain-key="effective coverage"><span>Effective coverage</span><strong>{pct(coverage?.effectiveCoveragePercent)}</strong><small>{coverage?.usableObservations ?? 0} usable of {coverage?.requested ?? 0} requested</small></article>
      <article data-explain-key="live coverage"><span>Live coverage</span><strong>{pct(coverage?.liveCoveragePercent)}</strong><small>{coverage?.liveFetched ?? 0} refreshed in current cycle</small></article>
      <article data-explain-key="last-known-good"><span>Last-known-good retained</span><strong>{coverage?.retainedLastKnownGood ?? 0}</strong><small>Preserved rather than replaced with blanks</small></article>
      <article data-explain-key="unresolved series"><span>Unresolved series</span><strong>{data.macro.failures.unresolved}</strong><small>{data.macro.failures.retryable} retryable · {data.macro.failures.nonRetryable} persistent</small></article>
      <article data-explain-key="market coverage"><span>Market coverage</span><strong>{data.market.priced}/{data.market.assets}</strong><small>{data.market.stale} retained stale quotes</small></article>
      <article data-explain-key="structure coverage"><span>Structure coverage</span><strong>{data.technical.assets}</strong><small>{data.technical.confirmed} confirmed · {data.technical.warming} building</small></article>
    </div>

    <div className="dq-two-col">
      <article className="panel dq-breakdown" data-explain-key="raw-data-quality"><div className="panel-title"><div><span className="eyebrow">Failure Classification</span><h2>Current macro exceptions</h2></div><span>{data.macro.failures.total}</span></div>
      ...
      <article className="panel dq-breakdown" data-explain-key="missing-data-coverage"><div className="panel-title"><div><span className="eyebrow">Economy Impact</span><h2>Coverage exceptions by economy</h2></div></div>
