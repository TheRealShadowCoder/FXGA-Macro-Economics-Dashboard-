...
type PolicyEconomy={economy:string;currency:string;centralBank:string;currentStance:string;dataPressure:number;currentPolicyEvidence:number;reactionGap:number;probabilities:{hike:number;hold:number;cut:number};pathPressure:{nextMeeting:number;threeMonth:number;sixMonth:number};nextMeeting:{event:string;date:string;forecast:string|null;previous:string|null}|null;scenarios:{inflationPersistence:number;growthScare:number;softLanding:number};confidence:number;interpretation:string;components:{growth:number;inflation:number;labour:number;financial:number;policy:number}};
