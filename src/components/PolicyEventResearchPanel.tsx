...
type PolicyMeeting={id?:string;event:string;date:string;forecast:string|null;previous:string|null;source?:string|null;providers?:string[];policyCatalystHorizonOnly?:boolean;policyProvenance?:{provider?:string;source?:string;retrievedAt?:string;windowStart?:string;windowEnd?:string}|null};
    ...
    <section className="policy-event-grid">
      ...
      <article className="panel policy-path-panel">
        ...
        <div className="policy-path-list">{policies.map(row=><div className="policy-path-row" key={row.currency}>
          ...
          {row.nextMeeting&&<div className="policy-meeting"><small>Next scheduled policy catalyst{row.nextMeeting.policyCatalystHorizonOnly?' · extended policy horizon':''}</small><span>{row.nextMeeting.event} · {new Date(row.nextMeeting.date).toLocaleString([], {month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}{row.nextMeeting.source?` · ${row.nextMeeting.source}`:''}</span></div>}
        </div>)}</div>
