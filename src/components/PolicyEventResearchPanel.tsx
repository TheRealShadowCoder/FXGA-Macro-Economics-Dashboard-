    ...
    <section className="policy-event-grid">
      ...
      <article className="panel policy-path-panel">
        ...
        <div className="policy-path-list">{policies.map(row=><div className="policy-path-row" key={row.currency}>
          ...
          {row.nextMeeting&&<div className="policy-meeting"><small>Next scheduled policy catalyst{row.nextMeeting.policyCatalystHorizonOnly?' · extended policy horizon':''}{row.nextMeeting.policyScheduleOnly?' · official schedule':''}</small><span>{row.nextMeeting.event} · {row.nextMeeting.timePrecision==='date-only'?new Date(row.nextMeeting.date).toLocaleDateString([], {month:'short',day:'numeric'}):new Date(row.nextMeeting.date).toLocaleString([], {month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}{row.nextMeeting.source?` · ${row.nextMeeting.source}`:''}{row.nextMeeting.timePrecision==='date-only'?' · time not asserted':''}</span></div>}
        </div>)}</div>
