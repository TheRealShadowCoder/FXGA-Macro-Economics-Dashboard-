...
function scheduleReconnect(){if(stopped)return;if(reconnectTimer!==null)clearTimeout(reconnectTimer);retry+=1;setState({state:'closed',reconnects:state.reconnects+1});const delay=jitter(Math.min(30_000,1000*(2**Math.min(retry,5))));reconnectTimer=window.setTimeout(()=>connectLive(),delay);}
