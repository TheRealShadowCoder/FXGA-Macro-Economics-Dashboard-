...
type View = 'overview' | 'analysis' | 'signals' | 'calendar' | 'indicators' | 'universe' | 'acquisition' | 'news' | 'sources';
  ...
  return (
    <div className="calendar-row">
      ...
      <div className="event-main">
        <div className="event-title"><span className={`impact i${event.importance}`}></span>{event.currency ? `${event.currency} · ` : ''}{event.event}</div>
        <div className="event-meta">
          {event.country} · {event.category} · {importanceLabel(event.importance)}
          {result && <span className={`outcome ${event.outcome}`}> · {result}</span>}
        </div>
        {event.analysisNote && <div className="event-analysis-note">{event.analysisNote}</div>}
      </div>
...
export default function App() {
  ...
  const filteredCalendar = useMemo(() => {
    ...
    return data.calendar.filter((e) => `${e.event} ${e.country} ${e.category} ${e.currency ?? ''}`.toLowerCase().includes(q));
  }, [data, query]);
