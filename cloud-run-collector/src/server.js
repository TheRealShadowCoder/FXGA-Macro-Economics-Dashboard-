...
const COUNTRY_CURRENCY = {
  US: 'USD', USA: 'USD', UK: 'GBP', GB: 'GBP', EMU: 'EUR', DE: 'EUR', FR: 'EUR', IT: 'EUR', ES: 'EUR',
  JP: 'JPY', CA: 'CAD', AU: 'AUD', NZ: 'NZD', CH: 'CHF', CN: 'CNY', ZA: 'ZAR',
};
const FXSTREET_COUNTRIES = ['US', 'UK', 'EMU', 'DE', 'CN', 'JP', 'CA', 'AU', 'NZ', 'CH', 'FR', 'IT', 'ES', 'ZA'];
...
const IMPORTANT_FRED = [
  'CPIAUCSL','CPILFESL','PCEPI','PCEPILFE','T5YIE','T10YIE','UNRATE','U6RATE','PAYEMS','CIVPART','ICSA','CCSA','JTSJOL','CES0500000003',
  'GDPC1','A191RL1Q225SBEA','INDPRO','CFNAI','WALCL','WRESBAL','RRPONTSYD','WTREGEN','M2SL','TOTBKCR','BUSLOANS','BAMLH0A0HYM2',
  'BAMLC0A4CBBB','DRTSCILM','FEDFUNDS','SOFR','DGS2','DGS10','T10Y2Y','T10Y3M','SAHMREALTIME','NFCI','STLFSI4','DTWEXBGS','VIXCLS','RSAFS'
].slice(0, 37);
...
function clean(value) {
  ...
  if (typeof value === 'object') {
    return clean(value.displayValue ?? value.value ?? value.Value ?? value.name ?? value.Name);
  }
...
async function fetchJson(url, timeoutMs = 5000) {
  ...
  try {
    const response = await fetch(url, { headers: { Accept: 'application/json', 'User-Agent': 'FXGA-Macro-Collector/1.0', Referer: 'https://www.fxstreet.com/economic-calendar' }, signal: controller.signal });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    return await response.json();
  } finally { clearTimeout(timer); }
}
async function fetchFxstreet(from, to) {
  return parseFxstreet(await fetchJson(fxstreetUrl(from,to), 4500));
}
...
function parseMyfxbookTable(html) {
  ...
  $('table tr').each((_, row) => {
    ...
    const currency = cells.find((value) => /^[A-Z]{3}$/.test(value));
    if (!currency) return;
...
async function scrapeMyfxbook() {
  ...
  try {
    const response = await fetch(url, { headers: { 'User-Agent':'Mozilla/5.0 FXGA-Macro-Collector/1.0', Accept:'text/html' }, redirect:'follow' });
    if (response.ok) {
      const parsed = parseMyfxbookTable(await response.text());
      if (parsed.length) return { events: parsed, mode:'html', ok:true };
    }
  } catch {}
  ...
  try {
    ...
    const events = parseMyfxbookTable(html);
...
async function scrapeCnbc() {
  ...
  try {
    const response = await fetch(url, { headers:{ 'User-Agent':'FXGA-Macro-Collector/1.0', Accept:'text/html' }, redirect:'follow' });
    ...
    $('a').each((_, anchor) => {
      ...
      if (text.length < 18 || !href || !/(econom|inflation|fed|jobs|gdp|rates|consumer|market)/i.test(text)) return;
...
function mergeEvents(primary, secondary) {
  ...
  for (const candidate of secondary) {
    const match = merged.find((event) => event.currency === candidate.currency
      && Math.abs(Date.parse(event.date)-Date.parse(candidate.date)) <= 5*60_000
...
async function getState(docName) {
  const snap = await state.doc(docName).get();
...
async function createReleaseTask(releaseAt, offsetSeconds, eventIds) {
  ...
  try {
    await tasks.createTask({ parent, task: { name:taskName,
      scheduleTime:{ seconds:Math.floor(scheduleMs/1000) },
      httpRequest:{ httpMethod:'POST', url:`${cfg.serviceUrl}/release-check`,
        headers:{ 'Content-Type':'application/json' }, body:Buffer.from(JSON.stringify({ releaseAt, offsetSeconds, eventIds })),
        oidcToken:{ serviceAccountEmail:cfg.taskInvokerSa, audience:cfg.serviceUrl } } } });
    return { created:true };
  } catch (error) {
    if (Number(error?.code) === 6) return { created:false, reason:'exists' };
    throw error;
  }
}
async function scheduleReleaseTasks(events) {
  ...
  for (const event of events) {
    ...
    cluster.push(event); clusters.set(key, cluster);
  }
  let created = 0;
  for (const [releaseAt, cluster] of clusters) {
    ...
    for (const offset of taskOffsets(maxImportance)) {
      const result = await createReleaseTask(releaseAt, offset, cluster.map((event)=>event.id));
...
async function signedWebhook(type, payload) {
  ...
  for (let attempt=0; attempt<3; attempt+=1) {
    try {
      const response = await fetch(cfg.webhookUrl, { method:'POST', headers:{ 'Content-Type':'application/json', 'X-FXGA-Timestamp':timestamp,
        'X-FXGA-Request-Id':requestId, 'X-FXGA-Signature':`sha256=${signature}` }, body });
...
async function bootstrapCalendar() {
  const now = new Date();
  const to = new Date(now.getTime()+cfg.calendarDays*86_400_000);
  ...
  const sourceHealth = {
    fxstreet:{ ok:fxstreet.length>0, events:fxstreet.length },
    myfxbook:{ ok:Boolean(myfx.ok), events:(myfx.events||[]).length, mode:myfx.mode, error:myfx.error },
    cnbc:{ ok:Boolean(cnbcResult.ok), items:(cnbcResult.items||[]).length, error:cnbcResult.error },
  };
  const snapshot = { generatedAt:new Date().toISOString(), days:cfg.calendarDays, events, sourceHealth, cnbcContext:cnbcResult.items||[] };
  const saved = await putIfChanged('calendar', snapshot);
  const scheduled = await scheduleReleaseTasks(events);
...
async function releaseCheck({ eventIds = [], releaseAt, offsetSeconds = 0 }) {
  const calendarState = await getState('calendar');
  const allEvents = calendarState?.payload?.events || [];
  ...
  let fresh = [];
  try { fresh = await fetchFxstreet(from,to); } catch {}
  const changed = [];
  const next = allEvents.map((event) => {
    ...
    const merged = { ...event, actual:candidate.actual??event.actual, forecast:candidate.forecast??event.forecast, previous:candidate.previous??event.previous,
      revised:candidate.revised??event.revised, deviation:candidate.deviation??event.deviation, lastUpdate:candidate.lastUpdate??new Date().toISOString(),
      providers:[...new Set([...(event.providers||[]),'fxstreet'])] };
    if (eventFingerprint(merged) !== eventFingerprint(event)) changed.push(merged);
    return merged;
  });
...
async function fetchFredSeries(seriesId) {
  ...
  url.searchParams.set('sort_order','desc'); url.searchParams.set('limit','18');
  ...
  return { seriesId, value:latest?.value??null, date:latest?.date??null, previous:previous?.value??null,
    change:latest&&previous?latest.value-previous.value:null, history };
}
async function syncFred() {
  if (!cfg.fredApiKey) return { skipped:true, reason:'FRED_API_KEY not configured' };
  const observations = [];
  for (let index=0; index<IMPORTANT_FRED.length; index+=5) {
    const settled = await Promise.allSettled(IMPORTANT_FRED.slice(index,index+5).map(fetchFredSeries));
    settled.forEach((result)=>{ if (result.status==='fulfilled') observations.push(result.value); });
  }
  const snapshot = { generatedAt:new Date().toISOString(), importantOnly:true, requested:IMPORTANT_FRED.length, observations };
  const saved = await putIfChanged('macro', snapshot);
  if (saved.changed) await signedWebhook('macro-snapshot', snapshot);
  return { changed:saved.changed, observations:observations.length, requested:IMPORTANT_FRED.length };
}

app.get('/health', async (_req,res) => {
  const [calendar,macro] = await Promise.all([getState('calendar'),getState('macro')]);
  res.json({ ok:true, service:'fxga-cloud-run-collector', projectConfigured:Boolean(cfg.projectId), tasksConfigured:Boolean(cfg.serviceUrl&&cfg.taskInvokerSa),
    webhookConfigured:Boolean(cfg.webhookSecret&&cfg.webhookUrl), fredConfigured:Boolean(cfg.fredApiKey), calendarUpdatedAt:calendar?.updatedAt??null,
    macroUpdatedAt:macro?.updatedAt??null, mode:'event-driven-cloud-run' });
});
...
app.post('/macro-sync', async (_req,res,next) => { try { res.json(await syncFred()); } catch (error) { next(error); } });
app.get('/state', async (_req,res) => {
  const [calendar,macro,sourceHealth] = await Promise.all([getState('calendar'),getState('macro'),getState('source-health')]);
  res.json({ calendar, macro, sourceHealth });
});
app.use((error,_req,res,_next) => { console.error(error); res.status(500).json({ error:String(error?.message||error).slice(0,1000) }); });

app.listen(cfg.port, () => console.log(`FXGA Cloud Run collector listening on :${cfg.port}`));
