...
function qualityValue(feature,row){const raw=Number(row?.featureVector?.[feature.id]);if(!Number.isFinite(raw))return null;const direction=String(row?.direction||'').toUpperCase()==='SELL'?-1:1;if(feature.mode==='directional')return clamp(direction*raw/feature.scale);if(feature.mode==='lower')return clamp(1-2*Math.max(0,Math.min(1,raw/feature.scale)));return clamp(2*Math.max(0,Math.min(1,raw/feature.scale))-1);}
...
function attributionFor(rows,feature){const values=[],outcomes=[];for(const row of rows){const value=qualityValue(feature,row);if(value==null)continue;values.push(value);outcomes.push(Math.tanh(Number(row.outcome.signedBps)/25));}const samples=values.length;if(!samples)return {id:feature.id,label:feature.label,samples:0,association:0,shrunkAssociation:0,status:'unproven',topMinusBottomBps:null};const association=correlation(values,outcomes),shrink=samples/(samples+14),shrunk=association*shrink;const ordered=values.map((value,i)=>({value,bps:Number(rows.filter(r=>qualityValue(feature,r)!=null)[i]?.outcome?.signedBps||0)})).sort((a,b)=>a.value-b.value),bucket=Math.max(1,Math.floor(ordered.length/3)),bottom=ordered.slice(0,bucket),top=ordered.slice(-bucket),spread=mean(top.map(x=>x.bps))-mean(bottom.map(x=>x.bps)),status=samples<8?'unproven':shrunk>=.10?'helpful':shrunk<=-.10?'harmful':'mixed';return {id:feature.id,label:feature.label,samples,association:Number(association.toFixed(4)),shrunkAssociation:Number(shrunk.toFixed(4)),status,topMinusBottomBps:Number(spread.toFixed(2))};}
