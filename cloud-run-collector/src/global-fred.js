...
export async function discoverGlobalFredUniverse(apiKey,fetchJson,options={}) {
  ...
  for (const [economy,searches] of Object.entries(ECONOMY_SEARCHES)) {
    for (const [category,searchText] of searches) {
      ...
      url.searchParams.set('search_text',searchText); url.searchParams.set('limit','12'); url.searchParams.set('order_by','search_rank');
      ...
      const candidates=(payload?.seriess||[])
        ...
        .sort((a,b)=>b.score-a.score).slice(0,maxPerQuery);
      for (const {series,score} of candidates) {
        if (FRED_BASE_IDS.includes(series.id)) continue;
        const descriptor={seriesId:series.id,title:series.title,units:series.units_short||series.units||'',frequency:series.frequency||'',seasonalAdjustment:series.seasonal_adjustment||'',lastUpdated:series.last_updated||'',economy,category,popularity:Number(series.popularity||0),discoveryScore:Number(score.toFixed(2)),source:'FRED dynamic economy discovery',curated:false};
  ...
  for (const economy of Object.keys(ECONOMY_SEARCHES)) {
    ...
    for (const item of ranked.slice(0,ECONOMY_MINIMUM[economy]||10)) {
  ...
  return [...curated,...selected].slice(0,maxSeries);
}

export function summarizeUniverse(universe) {
  const byEconomy={}; const byCategory={};
  ...
  return {total:universe.length,curatedBase:FRED_BASE_IDS.length,byEconomy,byCategory,economyMinimums:ECONOMY_MINIMUM};
}
