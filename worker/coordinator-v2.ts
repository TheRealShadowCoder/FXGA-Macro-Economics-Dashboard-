...
export class FxgaCoordinator extends DurableObject<Env> {
  ...
  private async globalMacroView() {
    const baseline = await this.ctx.storage.get<Record<string, any>>(COLLECTOR_BASELINE_KEY);
    const observations = Array.isArray(baseline?.observations) ? baseline.observations as MacroObservation[] : [];
    ...
    return {
      ...
      mode: this.externalMode() ? 'google-cloud-run-webhook' : 'cloudflare-native',
      ...
      economies,
      global,
      structuralUsAnalysis: baseline?.analysis ?? null,
    };
  }
  ...
  private async schedulerView(bootstrap = false) {
    ...
    if (bootstrap && scheduler.initialized && !calendarSourceStatus) {
      try { await syncCalendarSchedule(this.env, this.ctx.storage); } catch { /* status persisted on failure */ }
  ...
  private normalizeObservations(input: any[]): MacroObservation[] {
    return input.filter((item) => item && typeof item.seriesId === 'string').map((item) => ({
      ...
      economy: typeof item.economy === 'string' ? item.economy : undefined,
      economies: Array.isArray(item.economies) ? item.economies.map(String) : (typeof item.economy === 'string' ? [item.economy] : undefined),
      importance: item.importance === 'critical' ? 'critical' : 'high',
      source: typeof item.source === 'string' ? item.source : 'FRED',
  ...
  private async applyCollectorWebhook(requestId: string, envelope: Record<string, any>) {
    ...
    if (type === 'calendar-snapshot') {
      ...
      await this.ctx.storage.put(COLLECTOR_BASELINE_KEY, {
        ...
        importantOnly: true,
        dynamicInternational: Boolean(payload.dynamicInternational),
        targetEconomies: Array.isArray(payload.targetEconomies) ? payload.targetEconomies : TARGET_ECONOMIES,
        universeSummary: payload.universeSummary ?? null,
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/collector/global-macro') return Response.json(await this.globalMacroView());
