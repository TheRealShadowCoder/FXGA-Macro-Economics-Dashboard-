...
export function sourceRegistry(env: Env): SourceInfo[] {
  return [
    ...
    },
    {
      ...
      name: 'Myfxbook Economic Calendar Scraper',
      category: 'Primary Economic Calendar',
      region: 'Global',
      status: 'live',
      note: 'Primary low-cost schedule and release-value source. CSV export is preferred, public HTML table is fallback.',
    },
    {
      id: 'fxstreet-calendar',
      name: 'FXStreet Economic Calendar Scraper',
      category: 'Economic Calendar Confirmation',
      region: 'Global',
      status: env.BROWSER ? 'live' : 'error',
      note: 'Embedded/static extraction first; Cloudflare Playwright only when the public calendar requires JavaScript rendering.',
    },
    {
      id: 'cnbc-economy',
      name: 'CNBC Economic Schedule Corroborator',
      category: 'Calendar / News Confirmation',
      region: 'United States / Global',
      status: 'live',
      note: 'Lower-weight corroboration source. Only explicit dated/timed public CNBC schedule references are accepted; missing CNBC data never removes Myfxbook/FXStreet events.',
    },
    {
      id: 'trading-economics',
      name: 'Trading Economics Calendar (legacy optional)',
      category: 'Optional Legacy Calendar',
      region: 'Global',
      status: env.TRADING_ECONOMICS_API_KEY ? 'live' : 'needs_key',
      note: 'No longer required by the FXGA release scheduler. Retained only as an optional legacy adapter.',
    },
    {
      id: 'browser-run',
