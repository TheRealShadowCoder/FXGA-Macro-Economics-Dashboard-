...
const INITIAL_BARS_PER_ASSET=RETENTION_DAYS*24*60;
...
function decodeBlob(data){
  if(!data)return [];
  try{
    const source=Buffer.isBuffer(data)?data:typeof data.toBuffer==='function'?data.toBuffer():typeof data.toUint8Array==='function'?Buffer.from(data.toUint8Array()):Buffer.from(data);
...
function hasWeekendBetween(a,b){const start=new Date(a);start.setUTCHours(0,0,0,0);const end=new Date(b);end.setUTCHours(0,0,0,0);for(let t=start.getTime();t<=end.getTime();t+=86_400_000){const d=new Date(t).getUTCDay();if(d===0||d===6)return true;}return false;}
...
function diagnoseBars(rows){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    if(seen.has(ts))duplicateTimestamps++;
    seen.add(ts);
    if(i>0&&ts<Number(rows[i-1][0]))outOfOrder++;
  }
...
function subtractUtcDays(ms,days){return ms-days*86_400_000;}
...
function dateRangeIdsBetween(symbol,startMs,endMs){const ids=[];let cursor=new Date(startMs);cursor.setUTCHours(0,0,0,0);const end=new Date(endMs);end.setUTCHours(0,0,0,0);while(cursor.getTime()<=end.getTime()){ids.push(chunkId(symbol,dayKey(cursor.getTime())));cursor=new Date(cursor.getTime()+86_400_000);}return ids;}
...
function aggregateBars(rows,timeframe){
  ...
  for(const row of rows){
    const t=Math.floor(Number(row[0])/bucketMs)*bucketMs;let x=map.get(t);
...
function mergeSeriesChunk(row,d){
  ...
  next.symbol=d.symbol;next.timeframe=BASE_TIMEFRAME;next.bars+=Number(d.count||0);next.chunks+=1;next.compressedBytes+=Number(d.compressedBytes||0);next.rawBytes+=Number(d.rawBytes||0);
  next.oldestMs=next.oldestMs==null?Number(d.startMs):Math.min(next.oldestMs,Number(d.startMs||next.oldestMs));
  next.newestMs=next.newestMs==null?Number(d.endMs):Math.max(next.newestMs,Number(d.endMs||next.newestMs));
  if(d.brokerSymbol)next.brokerSymbol=d.brokerSymbol;
  if(d.latestClose!=null)next.latestClose=Number(d.latestClose);
  if(d.latestTickVolume!=null)next.latestTickVolume=Number(d.latestTickVolume);
  if(d.latestSpread!=null)next.latestSpread=Number(d.latestSpread);
  return next;
}
...
async function rebuildMeta(db,previous={}){
  ...
  for(const doc of snap.docs){
    const d=doc.data();totalCompressedBytes+=Number(d.compressedBytes||0);totalRawBytes+=Number(d.rawBytes||0);totalBars+=Number(d.count||0);
    const key=seriesKey(String(d.symbol||''));series[key]=mergeSeriesChunk(series[key],d);const prior=latestChunk.get(key);if(!prior||Number(d.endMs||0)>Number(prior.endMs||0))latestChunk.set(key,d);
  }
  for(const symbol of ALLOWED_SYMBOLS){
    const key=seriesKey(symbol),row=series[key]||emptySeries(symbol),old=previous.series?.[key]||{};
    for(const field of ['ingestBatches','receivedBars','acceptedBars','deduplicatedBars','rejectedBars','duplicateTimestamps','outOfOrderBars','gapEvents','missingCandleEstimate','maxGapMinutes','evictedChunks','evictedBars','evictedBytes','brokerSymbolChanges','wireBytesReceived','normalizedBytesReceived','normalizedCompressedBytesReceived'])if(old[field]!=null)row[field]=Number(old[field]||0);
    row.lastBrokerSymbolChangeAt=old.lastBrokerSymbolChangeAt??null;row.lastBatch=old.lastBatch??null;row.lastIngestAt=old.lastIngestAt??row.lastIngestAt??null;
    const latest=latestChunk.get(key);if(latest){const bars=decodeBlob(latest.payload),last=bars.at(-1);if(last){row.latestClose=Number(last[4]);row.latestTickVolume=Number(last[5]||0);row.latestSpread=Number(last[6]||0);}}
    series[key]=row;
  }
  const now=new Date().toISOString(),value={...previous,metaVersion:META_VERSION,schema:SCHEMA,retentionDays:RETENTION_DAYS,cacheEnvelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,evictTargetBytes:EVICT_TARGET_BYTES,totalCompressedBytes,totalRawBytes,totalBars,totalChunks:snap.size,series,reconciledAt:now,updatedAt:previous.updatedAt??now};
  await db.collection(META).doc(GLOBAL_ID).set(value,{merge:false});return value;
}

async function getMeta(db){
  const ref=db.collection(META).doc(GLOBAL_ID),snap=await ref.get();
  if(!snap.exists)return rebuildMeta(db,{});
  const data=snap.data();
  if(Number(data?.metaVersion||0)!==META_VERSION||data?.schema!==SCHEMA||Number(data?.retentionDays||0)!==RETENTION_DAYS)return rebuildMeta(db,data||{});
  return data;
}

function applyDeletedToSeries(meta,deleted){
  const series={...(meta.series||{})};
  for(const d of deleted){
    const key=seriesKey(String(d.symbol||'')),old=series[key]||emptySeries(String(d.symbol||''));
    series[key]={...old,bars:Math.max(0,Number(old.bars||0)-Number(d.count||0)),chunks:Math.max(0,Number(old.chunks||0)-1),compressedBytes:Math.max(0,Number(old.compressedBytes||0)-Number(d.compressedBytes||0)),rawBytes:Math.max(0,Number(old.rawBytes||0)-Number(d.rawBytes||0)),evictedChunks:Number(old.evictedChunks||0)+1,evictedBars:Number(old.evictedBars||0)+Number(d.count||0),evictedBytes:Number(old.evictedBytes||0)+Number(d.compressedBytes||0)};
  }
  return series;
}
...
async function reconcileSeriesBounds(db,meta,symbols){
  ...
  for(const symbol of symbols){
    const snap=await db.collection(CHUNKS).where('symbol','==',symbol).get(),preserved=series[seriesKey(symbol)]||emptySeries(symbol);
    let row={...emptySeries(symbol),...Object.fromEntries(['ingestBatches','receivedBars','acceptedBars','deduplicatedBars','rejectedBars','duplicateTimestamps','outOfOrderBars','gapEvents','missingCandleEstimate','maxGapMinutes','evictedChunks','evictedBars','evictedBytes','brokerSymbolChanges','wireBytesReceived','normalizedBytesReceived','normalizedCompressedBytesReceived'].map(field=>[field,Number(preserved[field]||0)])),lastBrokerSymbolChangeAt:preserved.lastBrokerSymbolChangeAt,lastBatch:preserved.lastBatch,lastIngestAt:preserved.lastIngestAt,brokerSymbol:preserved.brokerSymbol,latestClose:preserved.latestClose,latestTickVolume:preserved.latestTickVolume,latestSpread:preserved.latestSpread};
    for(const doc of snap.docs)row=mergeSeriesChunk(row,doc.data());
    series[seriesKey(symbol)]=row;
  }
  return {...meta,series};
}

async function deleteChunks(db,meta,docs,reason){
  if(!docs.length)return {meta,evictedBytes:0,evictedBars:0,evictedChunks:0};
  const deleted=[],affected=new Set();let evictedBytes=0,evictedRawBytes=0,evictedBars=0;
  for(let offset=0;offset<docs.length;offset+=400){
    const batch=db.batch();
    for(const doc of docs.slice(offset,offset+400)){
      const d=doc.data();batch.delete(doc.ref);deleted.push(d);affected.add(String(d.symbol||''));evictedBytes+=Number(d.compressedBytes||0);evictedRawBytes+=Number(d.rawBytes||0);evictedBars+=Number(d.count||0);
    }
    await batch.commit();
  }
  meta={...meta,totalCompressedBytes:Math.max(0,Number(meta.totalCompressedBytes||0)-evictedBytes),totalRawBytes:Math.max(0,Number(meta.totalRawBytes||0)-evictedRawBytes),totalBars:Math.max(0,Number(meta.totalBars||0)-evictedBars),totalChunks:Math.max(0,Number(meta.totalChunks||0)-deleted.length),evictedBytes:Number(meta.evictedBytes||0)+evictedBytes,evictedBars:Number(meta.evictedBars||0)+evictedBars,evictedChunks:Number(meta.evictedChunks||0)+deleted.length,lastEvictionAt:new Date().toISOString(),lastEvictionReason:reason,series:applyDeletedToSeries(meta,deleted)};
  if(affected.size)meta=await reconcileSeriesBounds(db,meta,[...affected].filter(Boolean));
  return {meta,evictedBytes,evictedBars,evictedChunks:deleted.length};
}

async function trimExpired(db,meta,nowMs=Date.now()){
  const cutoff=nowMs-RETENTION_MS;
  const snap=await db.collection(CHUNKS).where('endMs','<',cutoff).orderBy('endMs','asc').limit(400).get();
  if(snap.empty)return {meta,evictedBytes:0,evictedBars:0,evictedChunks:0,cutoff};
  const result=await deleteChunks(db,meta,snap.docs,'60-day-time-fifo');
...
function updateSeriesStorage(meta,symbol,{barDelta,chunkDelta,compressedDelta,rawDelta,startMs,endMs,brokerSymbol,latestBar}){
  const key=seriesKey(symbol),series={...(meta.series||{})},old=series[key]||emptySeries(symbol),changed=old.brokerSymbol&&brokerSymbol&&old.brokerSymbol!==brokerSymbol;
  series[key]={...old,symbol,timeframe:BASE_TIMEFRAME,bars:Math.max(0,Number(old.bars||0)+barDelta),chunks:Math.max(0,Number(old.chunks||0)+chunkDelta),compressedBytes:Math.max(0,Number(old.compressedBytes||0)+compressedDelta),rawBytes:Math.max(0,Number(old.rawBytes||0)+rawDelta),oldestMs:old.oldestMs==null?startMs:Math.min(Number(old.oldestMs),startMs),newestMs:old.newestMs==null?endMs:Math.max(Number(old.newestMs),endMs),lastIngestAt:new Date().toISOString(),brokerSymbol:brokerSymbol||old.brokerSymbol||symbol,brokerSymbolChanges:Number(old.brokerSymbolChanges||0)+(changed?1:0),lastBrokerSymbolChangeAt:changed?new Date().toISOString():old.lastBrokerSymbolChangeAt??null,latestClose:latestBar?.[4]??old.latestClose??null,latestTickVolume:latestBar?.[5]??old.latestTickVolume??null,latestSpread:latestBar?.[6]??old.latestSpread??null};
  return series;
}

function updateBatchStats(meta,symbol,stats){
  const key=seriesKey(symbol),series={...(meta.series||{})},old=series[key]||emptySeries(symbol);
  series[key]={...old,ingestBatches:Number(old.ingestBatches||0)+1,receivedBars:Number(old.receivedBars||0)+stats.receivedBars,acceptedBars:Number(old.acceptedBars||0)+stats.acceptedBars,deduplicatedBars:Number(old.deduplicatedBars||0)+stats.deduplicatedBars,rejectedBars:Number(old.rejectedBars||0)+stats.rejectedBars,duplicateTimestamps:Number(old.duplicateTimestamps||0)+stats.duplicateTimestamps,outOfOrderBars:Number(old.outOfOrderBars||0)+stats.outOfOrderBars,gapEvents:Number(old.gapEvents||0)+stats.gapEvents,missingCandleEstimate:Number(old.missingCandleEstimate||0)+stats.missingCandleEstimate,maxGapMinutes:Math.max(Number(old.maxGapMinutes||0),stats.maxGapMinutes),wireBytesReceived:Number(old.wireBytesReceived||0)+stats.wireBytes,normalizedBytesReceived:Number(old.normalizedBytesReceived||0)+stats.normalizedBytes,normalizedCompressedBytesReceived:Number(old.normalizedCompressedBytesReceived||0)+stats.normalizedCompressedBytes,lastBatch:{...stats,at:new Date().toISOString()}};
  return series;
}
...
function sizeProjection(row){
  ...
  const projectedRaw=rawPerBar==null?null:Math.round(rawPerBar*INITIAL_BARS_PER_ASSET),projectedCompressed=compressedPerBar==null?null:Math.round(compressedPerBar*INITIAL_BARS_PER_ASSET);
  return {exactStoredRawBytes:raw,exactStoredCompressedBytes:compressed,rawBytesPerBar:rawPerBar==null?null:round(rawPerBar,3),compressedBytesPerBar:compressedPerBar==null?null:round(compressedPerBar,3),wireBytesPerReceivedBar:wirePerBar==null?null:round(wirePerBar,3),projectedRawBytesAt20000:projectedRaw,projectedCompressedBytesAt20000:projectedCompressed,projectedRawBytesAtRetentionMax:projectedRaw,projectedCompressedBytesAtRetentionMax:projectedCompressed,projectedWireBytesFor160Bars:wirePerBar==null?null:Math.round(wirePerBar*INCREMENTAL_BARS_PER_SYNC)};
}
...
function enrichSeriesStatus(row,now=Date.now()){
  ...
  const retainedDays=r.oldestMs&&r.newestMs?round((r.newestMs-r.oldestMs)/86_400_000,2):0;
  return {...r,freshnessMs,freshnessMinutes:freshnessMs==null?null:round(freshnessMs/60_000,2),marketOpen,storagePercentOfCache:pct(r.compressedBytes,CACHE_ENVELOPE_BYTES),compressionRatio:r.rawBytes?round(Number(r.compressedBytes||0)/Number(r.rawBytes),4):null,retainedDays,retentionDays:RETENTION_DAYS,retentionProgressPercent:Math.min(100,round(retainedDays/RETENTION_DAYS*100,2)),integrityScore,health,alerts,size:sizeProjection(r),bootstrapProgressPercent:Math.min(100,round(retainedDays/RETENTION_DAYS*100,2)),bootstrapTargetBars:INITIAL_BARS_PER_ASSET};
}

function databaseHealth(series){
  const rows=Object.values(series||{}),rank={WAITING:5,STALE:4,DEGRADED:3,GOOD:2,MARKET_CLOSED:1,EXCELLENT:0};let worst='EXCELLENT';for(const row of rows)if((rank[row.health]??0)>(rank[worst]??0))worst=row.health;
  const assetsOnline=rows.filter(r=>r.bars>0).length,assetsHealthy=rows.filter(r=>['EXCELLENT','GOOD','MARKET_CLOSED'].includes(r.health)).length,integrityIssues=rows.reduce((sum,r)=>sum+(r.alerts?.length||0),0);
  return {state:worst,assetsOnline,assetsHealthy,assetsExpected:ALLOWED_SYMBOLS.size,pairsOnline:assetsOnline,pairsHealthy:assetsHealthy,pairsExpected:ALLOWED_SYMBOLS.size,integrityIssues};
}
...
export function createMT5PriceCache({db}){
  ...
  async function ingest(payload){
    ...
    let meta=await getMeta(db),accepted=0,deduplicated=0,evictedChunks=0,evictedBars=0;
    const ageTrim=await trimExpired(db,meta);meta=ageTrim.meta;evictedChunks+=ageTrim.evictedChunks;evictedBars+=ageTrim.evictedBars;
    const protectedIds=new Set([...groups.keys()].map(day=>chunkId(symbol,day)));
    ...
    meta.metaVersion=META_VERSION;meta.schema=SCHEMA;meta.retentionDays=RETENTION_DAYS;meta.series=updateBatchStats(meta,symbol,batchStats);meta.utilizationPercent=pct(meta.totalCompressedBytes,CACHE_ENVELOPE_BYTES);meta.payloadUtilizationPercent=pct(meta.totalCompressedBytes,PAYLOAD_HARD_BYTES);meta.freeEnvelopeBytes=Math.max(0,CACHE_ENVELOPE_BYTES-Number(meta.totalCompressedBytes||0));meta.freeToHardBytes=Math.max(0,PAYLOAD_HARD_BYTES-Number(meta.totalCompressedBytes||0));
    meta.policy={source:'MetaTrader5',canonicalDataset:'M1 only',retention:'rolling 60 calendar days by UTC-day FIFO with exact query cutoff',retentionDays:RETENTION_DAYS,reconstruction:'M5/M15/M30/H1/H4/D1 derived from retained M1 at query/SMC time',initialBarsPerAsset:INITIAL_BARS_PER_ASSET,initialBarsMeaning:'maximum 24x7 M1 observations in 60 calendar days; session-limited assets contain fewer bars',incrementalBarsPerSync:INCREMENTAL_BARS_PER_SYNC,syncSeconds:SYNC_SECONDS,allowedSymbols:[...ALLOWED_SYMBOLS],dedupe:'bar open time',compression:'gzip level 9',eviction:'time FIFO first; capacity FIFO second',absoluteEnvelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,evictionTargetBytes:EVICT_TARGET_BYTES,integrity:'ingest-time OHLC validation + gap/order/duplicate/staleness diagnostics'};
    await metaRef.set(meta,{merge:false});
    const seriesStatus=enrichSeriesStatus(meta.series[seriesKey(symbol)]||emptySeries(symbol));
    return {ok:true,schema:SCHEMA,symbol,brokerSymbol,timeframe:BASE_TIMEFRAME,retentionDays:RETENTION_DAYS,receivedBars,acceptedBars:accepted,deduplicatedBars:batchStats.deduplicatedBars,rejectedBars,gapEvents:batchStats.gapEvents,missingCandleEstimate:batchStats.missingCandleEstimate,evictedChunks,evictedBars,size:{wireBytes,normalizedBytes:incomingEncoded.raw.length,normalizedCompressedBytes:incomingEncoded.compressed.length,storedRawBytes:seriesStatus.rawBytes,storedCompressedBytes:seriesStatus.compressedBytes,rawBytesPerBar:seriesStatus.size.rawBytesPerBar,compressedBytesPerBar:seriesStatus.size.compressedBytesPerBar,projectedCompressedBytesAtRetentionMax:seriesStatus.size.projectedCompressedBytesAtRetentionMax},bootstrap:{targetDays:RETENTION_DAYS,targetBarsMaximum:INITIAL_BARS_PER_ASSET,retainedBars:seriesStatus.bars,retainedDays:seriesStatus.retainedDays,progressPercent:seriesStatus.retentionProgressPercent,complete:seriesStatus.retentionProgressPercent>=99||Boolean(payload.bootstrap_complete)},cache:{totalCompressedBytes:meta.totalCompressedBytes,totalRawBytes:meta.totalRawBytes,totalBars:meta.totalBars,totalChunks:meta.totalChunks,envelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,utilizationPercent:meta.utilizationPercent},receivedAt:new Date().toISOString()};
  }

  async function status(){
    let meta=await getMeta(db);const trimmed=await trimExpired(db,meta);meta=trimmed.meta;if(trimmed.evictedChunks)await metaRef.set(meta,{merge:false});
    const now=Date.now(),series={};for(const symbol of ALLOWED_SYMBOLS){const key=seriesKey(symbol),row=meta.series?.[key]||emptySeries(symbol);series[key]=enrichSeriesStatus(row,now);}
    const health=databaseHealth(series),totalCompressedBytes=Number(meta.totalCompressedBytes||0);
    return {...meta,series,databaseHealth:health,sizeCalculator:globalSizeCalculator(meta,series),retentionDays:RETENTION_DAYS,retentionCutoff:new Date(now-RETENTION_MS).toISOString(),cacheEnvelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,evictTargetBytes:EVICT_TARGET_BYTES,allowedSymbols:[...ALLOWED_SYMBOLS],websiteAssets:WEBSITE_ASSETS,baseTimeframe:BASE_TIMEFRAME,derivedTimeframes:Object.keys(TIMEFRAMES),initialBarsPerAsset:INITIAL_BARS_PER_ASSET,incrementalBarsPerSync:INCREMENTAL_BARS_PER_SYNC,syncSeconds:SYNC_SECONDS,utilizationPercent:pct(totalCompressedBytes,CACHE_ENVELOPE_BYTES),payloadUtilizationPercent:pct(totalCompressedBytes,PAYLOAD_HARD_BYTES),freeEnvelopeBytes:Math.max(0,CACHE_ENVELOPE_BYTES-totalCompressedBytes),freeToHardBytes:Math.max(0,PAYLOAD_HARD_BYTES-totalCompressedBytes),management:{governorState:totalCompressedBytes>=PAYLOAD_HARD_BYTES?'BLOCKING':totalCompressedBytes>=EVICT_TARGET_BYTES?'EVICTION_ZONE':totalCompressedBytes>=PAYLOAD_HARD_BYTES*.8?'WATCH':'ARMED',evictionArmed:true,timeFifoActive:true,retentionDays:RETENTION_DAYS,compressionActive:true,deduplicationActive:true,integrityMonitoring:true,canonicalM1Only:true,reconstructionOnDemand:true,nextAction:totalCompressedBytes>=EVICT_TARGET_BYTES?'Retire oldest M1 chunks before protected payload ceiling is threatened':`Maintain rolling ${RETENTION_DAYS}-day M1 history; retire expired UTC-day chunks automatically`}};
  }

  async function query({symbol,timeframe='M1',limit=1000,from=null,to=null}){
    symbol=String(symbol||'').toUpperCase();timeframe=String(timeframe||'M1').toUpperCase();limit=Math.min(MAX_QUERY_BARS,Math.max(1,Number(limit)||1000));
    if(!ALLOWED_SYMBOLS.has(symbol))throw Object.assign(new Error(`Unknown MT5 cache symbol ${symbol}`),{statusCode:400});
    if(!TIMEFRAMES[timeframe])throw Object.assign(new Error(`Unsupported derived timeframe ${timeframe}`),{statusCode:400});
    ...
    let fromMs=finite(from),toMs=finite(to);
    if(fromMs!=null&&fromMs<10_000_000_000)fromMs*=1000;if(toMs!=null&&toMs<10_000_000_000)toMs*=1000;
    if(fromMs!=null||toMs!=null){fromMs=Math.max(retentionCutoff,fromMs??retentionCutoff);toMs=Math.min(Date.now(),toMs??Date.now());if(toMs<fromMs)throw Object.assign(new Error('MT5 price query to must be greater than or equal to from'),{statusCode:400});}
    let ids;
    if(fromMs!=null&&toMs!=null)ids=dateRangeIdsBetween(symbol,fromMs,toMs);
    else{
      const rawBarsNeeded=Math.max(limit,limit*TIMEFRAMES[timeframe]+TIMEFRAMES[timeframe]*2),days=Math.min(RETENTION_DAYS+1,Math.max(3,Math.ceil(rawBarsNeeded/1440*1.7)+7));
      ids=dateRangeIds(symbol,series.newestMs,days);
    }
    const rows=[];
    ...
    const lower=fromMs??retentionCutoff,upper=toMs??Date.now();
    const dedup=[...new Map(rows.filter(row=>Number(row[0])>=lower&&Number(row[0])<=upper).map(row=>[Number(row[0]),row])).values()].sort((a,b)=>a[0]-b[0]),aggregated=aggregateBars(dedup,timeframe).slice(-limit);
    return {schema:SCHEMA,source:SOURCE,symbol,brokerSymbol:series.brokerSymbol||symbol,timeframe,baseTimeframe:BASE_TIMEFRAME,derived:timeframe!==BASE_TIMEFRAME,reconstructionSource:`canonical rolling ${RETENTION_DAYS}-day M1`,retentionDays:RETENTION_DAYS,count:aggregated.length,bars:aggregated,oldestMs:aggregated[0]?.[0]??null,newestMs:aggregated.at(-1)?.[0]??null,generatedAt:new Date().toISOString(),cache:{totalCompressedBytes:Number(meta.totalCompressedBytes||0),cacheEnvelopeBytes:CACHE_ENVELOPE_BYTES,utilizationPercent:pct(meta.totalCompressedBytes,CACHE_ENVELOPE_BYTES)}};
  }

  return {ingest,status,query,constants:{SCHEMA,BASE_TIMEFRAME,RETENTION_DAYS,CACHE_ENVELOPE_BYTES,PAYLOAD_HARD_BYTES,EVICT_TARGET_BYTES,INITIAL_BARS_PER_ASSET,INCREMENTAL_BARS_PER_SYNC,SYNC_SECONDS,MAX_BARS_PER_REQUEST,MAX_QUERY_BARS,ALLOWED_SYMBOLS:[...ALLOWED_SYMBOLS],WEBSITE_ASSETS,TIMEFRAMES:Object.keys(TIMEFRAMES)}};
}
