...
function mergeBars(existing,incoming){
  const map=new Map();
  for(const row of existing)if(Array.isArray(row)&&row[0])map.set(Number(row[0]),row);
  for(const row of incoming)map.set(Number(row[0]),row);
  return [...map.values()].sort((a,b)=>a[0]-b[0]);
}
function hasWeekendBetween(a,b){
  const start=new Date(a);start.setUTCHours(0,0,0,0);
  const end=new Date(b);end.setUTCHours(0,0,0,0);
  for(let t=start.getTime();t<=end.getTime();t+=86400_000){const d=new Date(t).getUTCDay();if(d===0||d===6)return true;}
  return false;
}
function diagnoseBars(rows){
  ...
  for(let i=1;i<uniqueBars.length;i++){
    const delta=Number(uniqueBars[i][0])-Number(uniqueBars[i-1][0]);if(delta<=60_000)continue;
    if(hasWeekendBetween(uniqueBars[i-1][0],uniqueBars[i][0]))continue;
    const gapMinutes=Math.max(0,Math.round(delta/60_000)-1);if(gapMinutes){gapEvents++;missingMinutes+=gapMinutes;maxGapMinutes=Math.max(maxGapMinutes,gapMinutes);}
  }
...
async function rebuildMeta(db){
  ...
  for(const doc of snap.docs){const d=doc.data();totalCompressedBytes+=Number(d.compressedBytes||0);totalRawBytes+=Number(d.rawBytes||0);totalBars+=Number(d.count||0);const key=seriesKey(String(d.symbol||''));series[key]=mergeSeriesChunk(series[key],d);}
  const now=new Date().toISOString();const value={schema:SCHEMA,cacheEnvelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,evictTargetBytes:EVICT_TARGET_BYTES,totalCompressedBytes,totalRawBytes,totalBars,totalChunks:snap.size,evictedChunks:0,evictedBars:0,evictedBytes:0,series,reconciledAt:now,updatedAt:now};await db.collection(META).doc(GLOBAL_ID).set(value,{merge:true});return value;
}
...
async function reconcileSeriesBounds(db,meta,symbols){
  ...
  for(const symbol of symbols){
    ...
    let row={...emptySeries(symbol),ingestBatches:preserved.ingestBatches,receivedBars:preserved.receivedBars,acceptedBars:preserved.acceptedBars,deduplicatedBars:preserved.deduplicatedBars,rejectedBars:preserved.rejectedBars,duplicateTimestamps:preserved.duplicateTimestamps,outOfOrderBars:preserved.outOfOrderBars,gapEvents:preserved.gapEvents,missingCandleEstimate:preserved.missingCandleEstimate,maxGapMinutes:preserved.maxGapMinutes,evictedChunks:preserved.evictedChunks,evictedBars:preserved.evictedBars,evictedBytes:preserved.evictedBytes,brokerSymbolChanges:preserved.brokerSymbolChanges,lastBrokerSymbolChangeAt:preserved.lastBrokerSymbolChangeAt,lastBatch:preserved.lastBatch,lastIngestAt:preserved.lastIngestAt,brokerSymbol:preserved.brokerSymbol,latestClose:preserved.latestClose,latestTickVolume:preserved.latestTickVolume,latestSpread:preserved.latestSpread};
    for(const doc of snap.docs)row=mergeSeriesChunk(row,doc.data());series[seriesKey(symbol)]=row;
  }
  return {...meta,series};
}
async function evictOldest(db,meta,requiredBytes,protectedIds=new Set()){
  let current=Number(meta.totalCompressedBytes||0),evictedBytes=0,evictedRawBytes=0,evictedBars=0,evictedChunks=0;const deleted=[],affectedSymbols=new Set();
  ...
  if(affectedSymbols.size)meta=await reconcileSeriesBounds(db,meta,[...affectedSymbols].filter(Boolean));return {meta,evictedBytes,evictedBars,evictedChunks};
}
function updateSeriesStorage(meta,symbol,{barDelta,chunkDelta,compressedDelta,rawDelta,startMs,endMs,brokerSymbol,latestBar}){
  ...
  series[key]={...old,symbol,timeframe:BASE_TIMEFRAME,bars:Math.max(0,Number(old.bars||0)+barDelta),chunks:Math.max(0,Number(old.chunks||0)+chunkDelta),compressedBytes:Math.max(0,Number(old.compressedBytes||0)+compressedDelta),rawBytes:Math.max(0,Number(old.rawBytes||0)+rawDelta),oldestMs:old.oldestMs==null?startMs:Math.min(Number(old.oldestMs),startMs),newestMs:old.newestMs==null?endMs:Math.max(Number(old.newestMs),endMs),lastIngestAt:new Date().toISOString(),brokerSymbol:brokerSymbol||old.brokerSymbol||symbol,brokerSymbolChanges:Number(old.brokerSymbolChanges||0)+(changed?1:0),lastBrokerSymbolChangeAt:changed?new Date().toISOString():old.lastBrokerSymbolChangeAt??null,latestClose:latestBar?.[4]??old.latestClose??null,latestTickVolume:latestBar?.[5]??old.latestTickVolume??null,latestSpread:latestBar?.[6]??old.latestSpread??null};return series;
}
function updateBatchStats(meta,symbol,stats){
  const key=seriesKey(symbol),series={...(meta.series||{})},old=series[key]||emptySeries(symbol);series[key]={...old,ingestBatches:Number(old.ingestBatches||0)+1,receivedBars:Number(old.receivedBars||0)+stats.receivedBars,acceptedBars:Number(old.acceptedBars||0)+stats.acceptedBars,deduplicatedBars:Number(old.deduplicatedBars||0)+stats.deduplicatedBars,rejectedBars:Number(old.rejectedBars||0)+stats.rejectedBars,duplicateTimestamps:Number(old.duplicateTimestamps||0)+stats.duplicateTimestamps,outOfOrderBars:Number(old.outOfOrderBars||0)+stats.outOfOrderBars,gapEvents:Number(old.gapEvents||0)+stats.gapEvents,missingCandleEstimate:Number(old.missingCandleEstimate||0)+stats.missingCandleEstimate,maxGapMinutes:Math.max(Number(old.maxGapMinutes||0),stats.maxGapMinutes),lastBatch:{...stats,at:new Date().toISOString()}};return series;
}
function forexMarketOpen(ms=Date.now()){const d=new Date(ms),day=d.getUTCDay(),hour=d.getUTCHours();if(day===6)return false;if(day===0)return hour>=21;if(day===5)return hour<22;return true;}
function enrichSeriesStatus(row,now=Date.now()){
  const r={...row},latest=Number(r.newestMs||0),freshnessMs=latest?Math.max(0,now-latest):null,marketOpen=forexMarketOpen(now),lastBatch=r.lastBatch||{},alerts=[];
  if(!r.bars)alerts.push('No retained M1 bars');if(marketOpen&&freshnessMs!=null&&freshnessMs>STALE_AFTER_MS)alerts.push(`Feed latency ${Math.round(freshnessMs/60_000)} minutes`);if(Number(lastBatch.rejectedBars||0)>0)alerts.push(`${lastBatch.rejectedBars} malformed OHLC bars rejected in latest batch`);if(Number(lastBatch.outOfOrderBars||0)>0)alerts.push(`${lastBatch.outOfOrderBars} out-of-order bars in latest batch`);if(Number(lastBatch.missingCandleEstimate||0)>0)alerts.push(`≈${lastBatch.missingCandleEstimate} missing M1 candles detected in latest continuous-session batch`);if(Number(r.brokerSymbolChanges||0)>0&&r.lastBrokerSymbolChangeAt)alerts.push(`Broker symbol changed ${r.brokerSymbolChanges} time(s)`);
  ...
  return {...r,freshnessMs,freshnessMinutes:freshnessMs==null?null:Number((freshnessMs/60_000).toFixed(2)),marketOpen,storagePercentOfCache:pct(r.compressedBytes,CACHE_ENVELOPE_BYTES),compressionRatio:r.rawBytes?Number((Number(r.compressedBytes||0)/Number(r.rawBytes)).toFixed(4)):null,retainedDays:r.oldestMs&&r.newestMs?Number(((r.newestMs-r.oldestMs)/86400_000).toFixed(2)):0,integrityScore,health,alerts};
}
function databaseHealth(series){
  const rows=Object.values(series||{}),rank={WAITING:5,STALE:4,DEGRADED:3,GOOD:2,MARKET_CLOSED:1,EXCELLENT:0};let worst='EXCELLENT';for(const row of rows)if((rank[row.health]??0)>(rank[worst]??0))worst=row.health;const pairsOnline=rows.filter(r=>r.bars>0).length,pairsHealthy=rows.filter(r=>['EXCELLENT','GOOD','MARKET_CLOSED'].includes(r.health)).length,integrityIssues=rows.reduce((sum,r)=>sum+(r.alerts?.length||0),0);return {state:worst,pairsOnline,pairsHealthy,pairsExpected:ALLOWED_SYMBOLS.size,integrityIssues};
}
...
export function createMT5PriceCache({db}){
  ...
  async function ingest(payload){
    ...
    const symbol=String(payload.symbol).toUpperCase(),brokerSymbol=String(payload.broker_symbol||symbol),receivedBars=payload.bars.length,normalized=payload.bars.map(normalizeBar).filter(Boolean),rejectedBars=receivedBars-normalized.length;if(!normalized.length)throw Object.assign(new Error('No valid bars in payload'),{statusCode:422});
    const diagnosis=diagnoseBars(normalized),groups=new Map();for(const bar of diagnosis.uniqueBars){const day=dayKey(bar[0]);if(!groups.has(day))groups.set(day,[]);groups.get(day).push(bar);}if(groups.size>MAX_DAYS_PER_REQUEST)throw Object.assign(new Error(`Payload spans more than ${MAX_DAYS_PER_REQUEST} UTC days`),{statusCode:422});
    ...
    for(const [day,incoming] of [...groups.entries()].sort((a,b)=>a[0].localeCompare(b[0]))){
      ...
      const now=new Date().toISOString(),startMs=merged[0][0],endMs=merged.at(-1)[0],latestBar=merged.at(-1);await ref.set({schema:SCHEMA,symbol,brokerSymbol,timeframe:BASE_TIMEFRAME,day,startMs,endMs,count:newCount,compressedBytes:encoded.compressed.length,rawBytes:encoded.raw.length,compressionRatio:encoded.raw.length?Number((encoded.compressed.length/encoded.raw.length).toFixed(4)):1,payload:encoded.compressed,updatedAt:now,createdAt:snap.data()?.createdAt??now},{merge:false});
      const chunkDelta=snap.exists?0:1,barDelta=newCount-oldCount;meta={...meta,totalCompressedBytes:Number(meta.totalCompressedBytes||0)+compressedDelta,totalRawBytes:Number(meta.totalRawBytes||0)+rawDelta,totalBars:Number(meta.totalBars||0)+barDelta,totalChunks:Number(meta.totalChunks||0)+chunkDelta,series:updateSeriesStorage(meta,symbol,{barDelta,chunkDelta,compressedDelta,rawDelta,startMs,endMs,brokerSymbol,latestBar}),lastIngestAt:now,updatedAt:now};
    }
    const batchStats={receivedBars,acceptedBars:accepted,deduplicatedBars:deduplicated+diagnosis.duplicateTimestamps,rejectedBars,duplicateTimestamps:diagnosis.duplicateTimestamps,outOfOrderBars:diagnosis.outOfOrder,gapEvents:diagnosis.gapEvents,missingCandleEstimate:diagnosis.missingMinutes,maxGapMinutes:diagnosis.maxGapMinutes};meta.series=updateBatchStats(meta,symbol,batchStats);meta.utilizationPercent=pct(meta.totalCompressedBytes,CACHE_ENVELOPE_BYTES);meta.payloadUtilizationPercent=pct(meta.totalCompressedBytes,PAYLOAD_HARD_BYTES);meta.freeEnvelopeBytes=Math.max(0,CACHE_ENVELOPE_BYTES-Number(meta.totalCompressedBytes||0));meta.freeToHardBytes=Math.max(0,PAYLOAD_HARD_BYTES-Number(meta.totalCompressedBytes||0));meta.policy={source:'MetaTrader5',baseTimeframe:BASE_TIMEFRAME,allowedSymbols:[...ALLOWED_SYMBOLS],dedupe:'bar open time',compression:'gzip level 9',eviction:'global FIFO oldest UTC-day chunk first',absoluteEnvelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,evictionTargetBytes:EVICT_TARGET_BYTES,integrity:'ingest-time OHLC validation + gap/order/duplicate/staleness diagnostics'};await metaRef.set(meta,{merge:false});
    return {ok:true,schema:SCHEMA,symbol,brokerSymbol,timeframe:BASE_TIMEFRAME,receivedBars,acceptedBars:accepted,deduplicatedBars:batchStats.deduplicatedBars,rejectedBars,gapEvents:batchStats.gapEvents,missingCandleEstimate:batchStats.missingCandleEstimate,evictedChunks,evictedBars,cache:{totalCompressedBytes:meta.totalCompressedBytes,totalRawBytes:meta.totalRawBytes,totalBars:meta.totalBars,totalChunks:meta.totalChunks,envelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,utilizationPercent:meta.utilizationPercent},receivedAt:new Date().toISOString()};
  }
  async function status(){
    const meta=await getMeta(db),now=Date.now(),series={};for(const symbol of ALLOWED_SYMBOLS){const key=seriesKey(symbol),row=meta.series?.[key]||emptySeries(symbol);series[key]=enrichSeriesStatus(row,now);}const health=databaseHealth(series),totalCompressedBytes=Number(meta.totalCompressedBytes||0);
    return {...meta,series,databaseHealth:health,cacheEnvelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,evictTargetBytes:EVICT_TARGET_BYTES,allowedSymbols:[...ALLOWED_SYMBOLS],baseTimeframe:BASE_TIMEFRAME,derivedTimeframes:Object.keys(TIMEFRAMES),utilizationPercent:pct(totalCompressedBytes,CACHE_ENVELOPE_BYTES),payloadUtilizationPercent:pct(totalCompressedBytes,PAYLOAD_HARD_BYTES),freeEnvelopeBytes:Math.max(0,CACHE_ENVELOPE_BYTES-totalCompressedBytes),freeToHardBytes:Math.max(0,PAYLOAD_HARD_BYTES-totalCompressedBytes),management:{governorState:totalCompressedBytes>=PAYLOAD_HARD_BYTES?'BLOCKING':totalCompressedBytes>=EVICT_TARGET_BYTES?'EVICTION_ZONE':totalCompressedBytes>=PAYLOAD_HARD_BYTES*.8?'WATCH':'ARMED',evictionArmed:true,compressionActive:true,deduplicationActive:true,integrityMonitoring:true,nextAction:totalCompressedBytes>=EVICT_TARGET_BYTES?'Oldest UTC-day chunks will be retired before protected payload ceiling is threatened':'Continue rolling ingestion'}};
  }
