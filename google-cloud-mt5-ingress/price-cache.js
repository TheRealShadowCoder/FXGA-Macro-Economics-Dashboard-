...
function mergeBars(existing,incoming){
  const map=new Map();for(const row of existing)if(Array.isArray(row)&&row[0])map.set(Number(row[0]),row);for(const row of incoming)map.set(Number(row[0]),row);
...
async function evictOldest(db,meta,requiredBytes,protectedIds=new Set()){
  ...
  while(current>target&&guard++<20){
    const snap=await db.collection(CHUNKS).orderBy('endMs','asc').limit(40).get();if(snap.empty)break;
    const batch=db.batch();let removed=0;
    for(const doc of snap.docs){if(protectedIds.has(doc.id))continue;const d=doc.data();batch.delete(doc.ref);current-=Number(d.compressedBytes||0);evictedBytes+=Number(d.compressedBytes||0);evictedRawBytes+=Number(d.rawBytes||0);evictedBars+=Number(d.count||0);evictedChunks++;removed++;if(current<=target)break;}
    if(!removed)break;await batch.commit();
  }
  ...
  return {meta,evictedBytes,evictedBars,evictedChunks};
}
function recomputeSeriesAfterWrite(meta,symbol,barDelta,chunkDelta,startMs,endMs,brokerSymbol){
  const key=seriesKey(symbol),series={...(meta.series||{})},old=series[key]||{symbol,timeframe:BASE_TIMEFRAME,bars:0,chunks:0,oldestMs:null,newestMs:null,lastIngestAt:null,brokerSymbol:null};
  series[key]={...old,symbol,timeframe:BASE_TIMEFRAME,bars:Math.max(0,Number(old.bars||0)+barDelta),chunks:Math.max(0,Number(old.chunks||0)+chunkDelta),oldestMs:old.oldestMs==null?startMs:Math.min(Number(old.oldestMs),startMs),newestMs:old.newestMs==null?endMs:Math.max(Number(old.newestMs),endMs),lastIngestAt:new Date().toISOString(),brokerSymbol:brokerSymbol||old.brokerSymbol||symbol};return series;
}
...
export function createMT5PriceCache({db}){
  ...
  async function status(){const meta=await getMeta(db);return {...meta,cacheEnvelopeBytes:CACHE_ENVELOPE_BYTES,payloadHardBytes:PAYLOAD_HARD_BYTES,evictTargetBytes:EVICT_TARGET_BYTES,allowedSymbols:[...ALLOWED_SYMBOLS],baseTimeframe:BASE_TIMEFRAME};}
  return {ingest,status,constants:{SCHEMA,BASE_TIMEFRAME,CACHE_ENVELOPE_BYTES,PAYLOAD_HARD_BYTES,EVICT_TARGET_BYTES,ALLOWED_SYMBOLS:[...ALLOWED_SYMBOLS]}};
}
