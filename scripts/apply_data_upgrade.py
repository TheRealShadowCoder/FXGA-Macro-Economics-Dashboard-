...
server_path = Path("cloud-run-collector/src/server-v2.js")
...
server = replace_once(server, "async function bootstrapCalendar() {", market_sync + "\nasync function bootstrapCalendar() {", "market synchronization function")
...
worker_path = Path("worker/index-v3.ts")
worker = worker_path.read_text(encoding="utf-8")
worker = replace_once(worker, "const INTELLIGENCE_KEY='google:intelligence';\nconst META_KEY='google:meta';", "const INTELLIGENCE_KEY='google:intelligence';\nconst MARKET_KEY='google:market';\nconst META_KEY='google:meta';", "worker market storage key")
worker = replace_once(
    worker,
...
worker = replace_once(worker, "return {calendar,macro,intelligence,meta,events,upcoming,active,recent,initialized:Boolean(events.length&&(macro?.observations?.length||intelligence?.globalMacro?.totalObservations))};}", "return {calendar,macro,intelligence,market,meta,events,upcoming,active,recent,initialized:Boolean(events.length&&(macro?.observations?.length||intelligence?.globalMacro?.totalObservations))};}", "worker state market return")
worker = replace_once(
    worker,
...
print("FXGA CNBC market and 7-day calendar upgrade applied successfully.")
