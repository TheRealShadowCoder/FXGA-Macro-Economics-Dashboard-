...
# Persist compact decision-state vectors and expose recent completed analogue candidates.
...
if 'function pairFeatureVector(' not in text:
  ...
  return {
    refinedScore:Number(pair?.refined?.score||0),directionalProbability:Math.max(Number(posterior.buy||0),Number(posterior.sell||0)),evidenceQuality:Number(pair?.quality?.score||0),uncertainty:Number(pair?.uncertainty?.score||0),scenarioRobustness:Number(pair?.scenarioRobustness?.score||0),contradictions:Number(pair?.contradictions?.count||0),reactionGap:Number(pair?.reactionFunctionGap?.differential||0),releaseDifferential:Number(pair?.temporalIntelligence?.releaseDifferential||0),revisionDifferential:Number(pair?.temporalIntelligence?.revisionDifferential||0),communicationGap:Number(pair?.temporalIntelligence?.communicationGap||0),transitionRisk:Number(pair?.transitionRisk?.maxRisk||0),independenceRatio:Number(pair?.evidenceIndependence?.independenceRatio??1),modelHealth:Number(pair?.modelHealth?.score||0),counterfactualShift:Number(pair?.counterfactual?.minimumAdverseShift||100),causalNet:Number(pair?.causalTransmission?.netTransmission||0)
  };
}
"""
...
# Add analogue comparison to decision governance.
...
if 'function historicalAnalogueControls(' not in text:
    fn="""function analogueVector({refined,bayesian,quality,uncertainty,scenario,contradictions,reactionFunctionGap,temporal,transitionRisk,evidenceIndependence,modelHealth,counterfactual,causalTransmission}){
  const p=bayesian?.posterior||{};return {refinedScore:Number(refined?.score||0),directionalProbability:Math.max(Number(p.buy||0),Number(p.sell||0)),evidenceQuality:Number(quality?.score||0),uncertainty:Number(uncertainty?.score||0),scenarioRobustness:Number(scenario?.score||0),contradictions:Number(contradictions?.count||0),reactionGap:Number(reactionFunctionGap?.differential||0),releaseDifferential:Number(temporal?.releaseDifferential||0),revisionDifferential:Number(temporal?.revisionDifferential||0),communicationGap:Number(temporal?.communicationGap||0),transitionRisk:Number(transitionRisk?.maxRisk||0),independenceRatio:Number(evidenceIndependence?.independenceRatio??1),modelHealth:Number(modelHealth?.score||0),counterfactualShift:Number(counterfactual?.minimumAdverseShift||100),causalNet:Number(causalTransmission?.netTransmission||0)};
}
const ANALOGUE_SCALES={refinedScore:100,directionalProbability:1,evidenceQuality:100,uncertainty:100,scenarioRobustness:100,contradictions:5,reactionGap:100,releaseDifferential:100,revisionDifferential:100,communicationGap:100,transitionRisk:100,independenceRatio:1,modelHealth:100,counterfactualShift:100,causalNet:100};
...
# Add analogue factor into confidence formula if not already present.
if 'analogueFactor=Number(controls?.historicalAnalogues?.factor||1)' not in text:
    text=replace_once(text,
        ...
        "changeFactor=Number(controls?.decisionChange?.factor||1),analogueFactor=Number(controls?.historicalAnalogues?.factor||1),",
        'analogue factor declaration')
    text=replace_once(text,
        "*historyFactor*horizonFactor*changeFactor*",
        "*historyFactor*horizonFactor*changeFactor*analogueFactor*",
        'analogue confidence multiplication')

# Add adverse analogue veto.
...
# Construct current vector and analogue control before finalDecision.
if 'historicalAnalogues=historicalAnalogueControls(' not in text:
    marker="historicalCalibration=historicalCalibrationControls(decisionMemory,opportunity.symbol),"
    addition="historicalCalibration=historicalCalibrationControls(decisionMemory,opportunity.symbol),"
    # Later objects are already constructed on the same const chain; insert analogue after uncertainty is built so all inputs exist.
    text=replace_once(text,
        "premortem=buildPremortem(opportunity,contradictions,expectationGap,scenario,risk,quality),thesis=thesisForPair",
        "analogueState=analogueVector({refined,bayesian,quality,uncertainty,scenario,contradictions,reactionFunctionGap,temporal,transitionRisk,evidenceIndependence,modelHealth,counterfactual,causalTransmission}),historicalAnalogues=historicalAnalogueControls(decisionMemory,opportunity.symbol,preferred,analogueState),premortem=buildPremortem(opportunity,contradictions,expectationGap,scenario,risk,quality),thesis=thesisForPair",
        'analogue pair construction')

# Pass and expose analogue fields.
if 'historicalCalibration,horizonCalibration,decisionChange,historicalAnalogues' not in text:
    text=replace_once(text,
        'historicalCalibration,horizonCalibration,decisionChange,',
        'historicalCalibration,horizonCalibration,decisionChange,historicalAnalogues,',
...
if 'modelHealth,historicalCalibration,horizonCalibration,decisionChange,historicalAnalogues,uncertainty' not in text:
    text=replace_once(text,
        'modelHealth,historicalCalibration,horizonCalibration,decisionChange,uncertainty',
        'modelHealth,historicalCalibration,horizonCalibration,decisionChange,historicalAnalogues,uncertainty',
...
path.write_text(text,encoding='utf-8')
print('Historical analogue reasoning upgrade applied.')
