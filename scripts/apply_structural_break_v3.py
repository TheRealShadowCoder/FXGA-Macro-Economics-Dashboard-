...
if 'const structuralBreaks=buildStructuralBreaks(observations);' not in text:
    text=replace_once(
        ...
        'research structural-break build'
    )

if 'dataQuality,sourceReliability,evidenceIndependence,modelHealth,structuralBreaks,turningPoints,catalystSequence' not in text:
    text=replace_once(
        text,
        'dataQuality,sourceReliability,evidenceIndependence,modelHealth,turningPoints,catalystSequence',
        'dataQuality,sourceReliability,evidenceIndependence,modelHealth,structuralBreaks,turningPoints,catalystSequence',
        'research structural-break output'
    )
...
if 'structuralBreak=structuralBreakControls(research,opportunity.symbol)' not in text:
    text=replace_once(
        text,
        'temporal=temporalIntelligenceForPair(opportunity,events,news,economies,nowMs),transitionRisk=transitionRiskControls(research,opportunity.symbol),evidenceIndependence=evidenceIndependenceControls',
        'temporal=temporalIntelligenceForPair(opportunity,events,news,economies,nowMs),transitionRisk=transitionRiskControls(research,opportunity.symbol),structuralBreak=structuralBreakControls(research,opportunity.symbol),evidenceIndependence=evidenceIndependenceControls',
        'structural-break pair construction'
    )

# Evidence-completeness signature: anchor the whole function prefix so only the declaration is changed.
old_sig="function evidenceCompletenessControls({quality,causalTransmission,counterfactual,scenario,modelHealth,evidenceIndependence,temporal,marketSignal,crossAsset,historicalCalibration,transitionRisk,reactionFunctionGap}){"
new_sig="function evidenceCompletenessControls({quality,causalTransmission,counterfactual,scenario,modelHealth,evidenceIndependence,temporal,marketSignal,crossAsset,historicalCalibration,transitionRisk,structuralBreak,reactionFunctionGap}){"
if new_sig not in text:
    text=replace_once(text,old_sig,new_sig,'completeness structural-break signature')
...
old_complete_call="evidenceCompleteness=evidenceCompletenessControls({quality,causalTransmission,counterfactual,scenario,modelHealth,evidenceIndependence,temporal,marketSignal,crossAsset,historicalCalibration,transitionRisk,reactionFunctionGap})"
new_complete_call="evidenceCompleteness=evidenceCompletenessControls({quality,causalTransmission,counterfactual,scenario,modelHealth,evidenceIndependence,temporal,marketSignal,crossAsset,historicalCalibration,transitionRisk,structuralBreak,reactionFunctionGap})"
if new_complete_call not in text:
    text=replace_once(text,old_complete_call,new_complete_call,'completeness structural-break call')

# Historical analogue declaration and current-state vector.
old_analogue_sig="function analogueVector({refined,bayesian,quality,uncertainty,scenario,contradictions,reactionFunctionGap,crossAsset,evidenceCompleteness,temporal,transitionRisk,evidenceIndependence,modelHealth,counterfactual,causalTransmission}){"
new_analogue_sig="function analogueVector({refined,bayesian,quality,uncertainty,scenario,contradictions,reactionFunctionGap,crossAsset,evidenceCompleteness,temporal,transitionRisk,structuralBreak,evidenceIndependence,modelHealth,counterfactual,causalTransmission}){"
if new_analogue_sig not in text:
    text=replace_once(text,old_analogue_sig,new_analogue_sig,'analogue structural-break signature')
...
old_analogue_call="analogueState=analogueVector({refined,bayesian,quality,uncertainty,scenario,contradictions,reactionFunctionGap,crossAsset,evidenceCompleteness,temporal,transitionRisk,evidenceIndependence,modelHealth,counterfactual,causalTransmission})"
new_analogue_call="analogueState=analogueVector({refined,bayesian,quality,uncertainty,scenario,contradictions,reactionFunctionGap,crossAsset,evidenceCompleteness,temporal,transitionRisk,structuralBreak,evidenceIndependence,modelHealth,counterfactual,causalTransmission})"
if new_analogue_call not in text:
    text=replace_once(text,old_analogue_call,new_analogue_call,'analogue structural-break call')

# Final controls, pair output and governed audit use unique surrounding sequences.
if 'counterfactual,temporal,transitionRisk,structuralBreak,evidenceIndependence,evidenceCompleteness,uncertainty' not in text:
    text=replace_once(
        text,
...
if 'temporalIntelligence:temporal,transitionRisk,structuralBreak,evidenceIndependence,evidenceCompleteness' not in text:
    text=replace_once(
        text,
...
path.write_text(text,encoding='utf-8')
print('Structural-break v3 integration applied.')
